...
**Исследование показало:** План некорректен.

**Обоснование:**
- `format!("...").to_socket_addrs()` требует трейт `ToSocketAddrs` в scope
- Без этого импорта код не компилируется (E0599)
- Трейт ДОЛЖЕН быть импортирован для работы метода

**Выполнено:**
- [x] Не требует изменений — импорт нужен для компиляции
...
- Axum клонирует State **на каждый запрос** (через `State<T>` extractor)
- Это не проблема — всё внутри `AppState` обёрнуто в `Arc`/`Arc<Mutex>`
- `channels.clone()` не создаёт новые senders — Arc делает их shared
- Клонирование дешёвое (pointer copy)

**Вывод:**

- [x] Clone на Channels **необходим** — Axum требует для `.with_state()`
- [x] Текущая архитектура корректна — всё через Arc, клонирование безопасно
- [x] Не требует изменений
...
| 11  | Исследовать Clone на Channels     | [x]      |
...
Шаг 8 не требует изменений — проблема была некорректно сформулирована, `&mut self` правильный паттерн.
Шаг 9 не требует изменений — импорт ToSocketAddrs необходим для компиляции.
Шаг 10 добавлен для восстановления чистой архитектуры: `gamepad/input.rs` должен содержать input driver (gilrs loop).
Шаг 11 не требует изменений — Clone необходим для Axum, архитектура корректна.
